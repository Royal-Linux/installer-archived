# Error codes:
# * Exit 1: Failed to update pacman keys
# * Exit 2: Can't download update with wget - no internet connection
...
config() {
    ...
    # Log and tmp files
    log=/tmp/archroyal.log
    tmp_menu=/tmp/archroyal/part.sh
    tmp_list=/tmp/archroyal/part.list
    tmp_passwd=/tmp/archroyal/passwd
    ...
    case "${opt}" in
        ...
        ;;
        -k|--keys)
            echo -e "${Yellow}*> ArchRoyal: Updating pacman keys..."
            pacman-db-upgrade
            pacman-key --init
        ...
        -u|--update)
            tmp_dir=$(mktemp -d)
            echo -ne "\n${Yellow}*> ArchRoyal: Downloading..."
            wget -q -4 --no-check-certificate -O ${tmp_dir}/master.tar.gz https://github.com/ArchRoyalLinux/installer/archive/master.tar.gz
            if [ "$?" -gt "0" ]; then
            ...
            cp ${tmp_dir}/installer-master/wallpapers/* /usr/share/archroyal/extra/wallpapers/
            cp -f ${tmp_dir}/installer-master/extra/{sysinfo,iptest} /usr/bin/
            cp -r ${tmp_dir}/installer-master/extra/* /usr/share/archroyal/extra/
            echo -e "${Green}done"

            echo -e "\n${Green}*> ${Yellow}ArchRoyal updated successfully, you may now run archroyal ${ColorOff}"
            exit
        ;;
    esac
...
usage() {
    clear
    echo -e "Usage: ${Green} archroyal [options]${ColorOff}"
    ...
    echo -e "${Green}   -u | --update       ${Yellow}Update archroyal scripts${ColorOff}"
    echo -e ""
    echo -e "Installer:"
    echo -e "\t${Blue}┌--------------------------┬----------------------------┐"
