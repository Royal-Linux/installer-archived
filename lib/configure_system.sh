...
configure_system() {
  ...
  if "$enable_xfs" ; then
    ...
    if "$desktop"; then
        config_env &
        pid=$! pri="0.1" msg="$wait_load \n\n \Z1> \Z2archroyal configure desktop\Zn" load

        if [ "$DM" == "lightdm" ]; then
            cp -r "${archroyal_directory}"/extra/desktop/lightdm/lightdm-gtk-greeter.conf "$ARCH"/etc/lightdm/
        fi
    fi
    ...
    if "$aa_repo" ; then
        sed -i -e '$a\\n[archroyal]\nServer = https://archroyallinux.org/repo/x86_64\nSigLevel = Never' "$ARCH"/etc/pacman.conf
    fi
    ...
    if [ "$sh" == "/bin/bash" ]; then
        ...
        if [ "$shrc" == "$default" ]; then
            cp "${archroyal_directory}"/extra/.zshrc "$ARCH"/root/.zshrc
            cp "${archroyal_directory}"/extra/.zshrc "$ARCH"/etc/skel/.zshrc
        elif [ "$shrc" == "oh-my-zsh" ]; then
            cp "${archroyal_directory}"/extra/.zshrc-oh-my "$ARCH"/root/.zshrc
            cp "${archroyal_directory}"/extra/.zshrc-oh-my "$ARCH"/etc/skel/.zshrc
        elif [ "$shrc" == "grml-zsh-config" ]; then
            cp "${archroyal_directory}"/extra/.zshrc-grml "$ARCH"/root/.zshrc
            cp "${archroyal_directory}"/extra/.zshrc-grml "$ARCH"/etc/skel/.zshrc
        else
            touch "$ARCH"/root/.zshrc
            touch "$ARCH"/etc/skel/.zshrc
        fi
    elif [ "$shell" == "fish" ]; then
        echo "exec fish" >> "${archroyal_directory}"/extra/.bashrc-root
        echo "exec fish" >> "${archroyal_directory}"/extra/.bashrc
    elif [ "$shell" == "tcsh" ]; then
        cp "${archroyal_directory}"/extra/{.tcshrc,.tcshrc.conf} "$ARCH"/root/
        cp "${archroyal_directory}"/extra/{.tcshrc,.tcshrc.conf} "$ARCH"/etc/skel/
    elif [ "$shell" == "mksh" ]; then
        cp "${archroyal_directory}"/extra/.mkshrc "$ARCH"/root/
        cp "${archroyal_directory}"/extra/.mkshrc "$ARCH"/etc/skel/
    fi

    cp "${archroyal_directory}"/extra/.bashrc-root "$ARCH"/root/.bashrc
    cp "${archroyal_directory}"/extra/.bashrc "$ARCH"/etc/skel/
