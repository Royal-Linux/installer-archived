...
graphics() {
	...
	while (true); do
		...
		if [ -z "$de" ]; then
			...
			de=$(dialog --ok-button "$done_msg" --cancel-button "$back" --menu "$environment_msg" 15 60 5 \
				"ArchRoyal-budgie" "$de24" \
				"ArchRoyal-cinnamon" "$de23" \
				"ArchRoyal-gnome" "$de22" \
				"ArchRoyal-openbox" "$de18" \
				"ArchRoyal-xfce4" "$de15" 3>&1 1>&2 2>&3)
	...
	if ! (grep </etc/pacman.conf "ArchRoyal-local"); then
